...
import type { TokenSearchProvider, TokenSearchResult } from "../types.js";
...
const pctToBps = (pct: number): bigint => BigInt(Math.round(pct * 100));
 ...
 */
function usdToScaled(value: number, scale: bigint, scaleDigits: number): bigint {
...
export class JupiterTokenSearchProvider implements TokenSearchProvider {
  ...
  async getByMint(mint: string, signal?: AbortSignal): Promise<TokenSearchResult | null> {
  ...
  private async fetchSearch(query: string, signal?: AbortSignal): Promise<TokenSearchResult[]> {
