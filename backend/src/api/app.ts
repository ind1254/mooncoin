...
import type { TokenEvidenceSnapshot } from "../evidence/types.js";
import { assessRisk, type RiskAssessment } from "../risk/engineV3.js";
...
function serializeRiskAssessment(risk: RiskAssessment): Record<string, unknown> {
  return {
 ...
 */
function serializeEvidenceSummary(snapshot: TokenEvidenceSnapshot): Record<string, unknown> {
  return {
  ...
  app.get("/v1/tokens/:mint/history", async (req, res) => {
    try {
      ...
      res.json({
        ...
        count: points.length,
        notice:
          "Older points are downsampled: every observation for the last 6 hours, then hourly, then daily. Each point is a real observation, never an average.",
        points: points.map((p) => ({
          ...
          walletConcentrationPct:
            p.walletConcentrationBps === null ? null : pctStr(p.walletConcentrationBps),
  ...
  app.get("/v1/tokens/:mint/risk-change", async (req, res) => {
    try {
      ...
      const parsed = z
        ...
        .safeParse(req.query);
      if (!parsed.success) throw new ArbError("VALIDATION_ERROR", "Invalid risk-change query", 400);
      ...
      const current = assessRisk(
        snapshotFromResearch(profile, now, { maxMarketAgeMs: tradabilityPolicy.maxMarketAgeMs }),
        {
          minLiquidityUsdMicro: tradabilityPolicy.minLiquidityUsdMicro,
          maxPriceImpactBps: tradabilityPolicy.maxPriceImpactBps,
        },
  ...
  app.get("/v1/auto-watch", async (_req, res) => {
    try {
      ...
      res.json({
        ...
        notice:
          "Tokens here are tracked because they are established or scored well. Only mature coins leave discovery; quality matches stay visible. This is research context, not a recommendation.",
