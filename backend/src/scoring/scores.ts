import type { RouteComparison, TokenMarketView } from "../market/types.js";

/**
 * Transparent rule-based scoring.
 *
 * Every score is 0–100 and carries the exact factors that produced it, so the
 * UI can always answer "why does this token rank here?". Scores are decision
 * aids describing CURRENT conditions — they are never a prediction of future
 * returns, and the UI must not present them as one.
 *
 * Plain numbers are fine in this module: scores are qualitative indicators,
 * not financial amounts. All financial arithmetic stays bigint elsewhere.
 */
...
export type OpportunityLabel = "strong" | "moderate" | "weak" | "avoid";

export interface TokenScores {
  momentum: PillarScore;
  liquidity: PillarScore;
  execution: PillarScore;
  /** Higher = riskier (0 safest, 100 worst). */
  risk: PillarScore;
  opportunity: PillarScore;
  riskLevel: RiskLevel;
  opportunityLabel: OpportunityLabel;
}

/** Limits that come from user settings and shape execution/liquidity scoring. */
...
export function scoreMomentum(view: TokenMarketView): PillarScore {
...
export function scoreLiquidity(view: TokenMarketView, tradeSizeUsdMicro: bigint, limits: ScoringLimits): PillarScore {
  const l = view.liquidity.value;
  const factors: ScoreFactor[] = [];
  const liqUsd = Number(l.totalUsdMicro / 1_000_000n);
  ...
  if (l.totalUsdMicro < limits.minLiquidityUsdMicro) {
    score -= 15;
    factors.push({ id: "below-user-min", label: "Below your minimum", detail: `Liquidity is under your configured minimum of $${(Number(limits.minLiquidityUsdMicro / 1_000_000n)).toLocaleString()}`, direction: "negative" });
  }
  ...
  if (l.totalUsdMicro > 0n) {
    const ratioBps = Number((tradeSizeUsdMicro * 10_000n) / l.totalUsdMicro);
    if (ratioBps >= 100) {
      score -= 10;
...
export function scoreExecution(routes: RouteComparison | null, limits: ScoringLimits, nowMs: number): PillarScore {
  const factors: ScoreFactor[] = [];
...
export function scoreRisk(view: TokenMarketView): PillarScore {
...
export function computeScores(
  ...
  const riskLevel: RiskLevel =
    risk.score < riskBands.lowBelow ? "low" : risk.score < riskBands.mediumBelow ? "medium" : "high";
  ...
  const opportunityLabel: OpportunityLabel =
    rounded >= labels.strongAtLeast ? "strong" : rounded >= labels.moderateAtLeast ? "moderate" : rounded >= labels.weakAtLeast ? "weak" : "avoid";
