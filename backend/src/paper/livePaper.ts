...
import {
  ...
  PortfolioRepository,
  type LivePaperPositionRecord,
} from "../db/repositories.js";
import type { SqlClient } from "../db/client.js";
import type { NormalizedSwapQuote, QuoteProvider } from "../market/jupiter/quotes.js";
import { USDC_MINT, type TradabilityService } from "../market/tradability.js";

export interface LivePaperTradingConfig {
  startingMicroUsd: bigint;
  minTradeMicroUsd: bigint;
  maxTradeMicroUsd: bigint;
  maxOpenPositions: number;
  maxEntryPriceImpactBps: bigint;
}

export interface PaperPositionValuation {
  status: "fresh" | "unavailable" | "pending";
...
function averagePricePicoUsd(valueMicroUsd: bigint, quantityBaseUnits: bigint, decimals: number): bigint {
  if (quantityBaseUnits <= 0n) return 0n;
  return (valueMicroUsd * 1_000_000n * 10n ** BigInt(decimals)) / quantityBaseUnits;
}

function assertFreshQuote(
...
function paperPositionView(record: LivePaperPositionRecord, valuation?: PaperPositionValuation): Record<string, unknown> {
  const effectiveValuation: PaperPositionValuation =
  ...
  return {
    ...
    marketValueUsd:
      effectiveValuation.valueMicroUsd === null ? null : microToUsdString(effectiveValuation.valueMicroUsd),
    pnlUsd: pnlMicroUsd === null ? null : microToUsdString(pnlMicroUsd),
    returnPct,
    entry: {
      averagePriceUsd: picoUsdToPriceString(
        averagePricePicoUsd(record.entryCostMicroUsd, record.tokenQuantityBaseUnits, record.tokenDecimals),
      ),
    ...
    exit:
      record.status === "closed"
        ? {
            ...
            averagePriceUsd: picoUsdToPriceString(
              averagePricePicoUsd(
                record.closeProceedsMicroUsd!,
                record.tokenQuantityBaseUnits,
...
export class LivePaperTradingService {
  private readonly positions: LivePaperPositionRepository;
  private readonly portfolios: PortfolioRepository;

  constructor(
    private readonly db: SqlClient,
    private readonly tradability: TradabilityService,
    private readonly quotes: QuoteProvider,
    private readonly config: LivePaperTradingConfig,
  ...
  private parseEntryAmount(amountUsd: string): bigint {
    ...
    if (amount < this.config.minTradeMicroUsd || amount > this.config.maxTradeMicroUsd) {
      throw new ArbError(
        "AMOUNT_OUT_OF_RANGE",
        `Paper entries must be between $${microToUsdString(this.config.minTradeMicroUsd)} and $${microToUsdString(this.config.maxTradeMicroUsd)}.`,
        400,
      );
    }
    return amount;
  ...
  async openPosition(
    ...
    if (!check.eligible || !check.quote) {
      throw new ArbError(
        ...
        "This token did not pass every required production gate for that paper entry.",
        409,
    ...
    const record = await this.positions.open(
      userId,
  ...
  private async valueOpenPosition(record: LivePaperPositionRecord): Promise<PaperPositionValuation> {
    try {
      const quote = await this.quotes.getQuote({
      ...
      return {
        ...
        quote,
        detail:
          quote.priceImpactBps > this.config.maxEntryPriceImpactBps
  ...
  async getPortfolio(userId: string): Promise<Record<string, unknown>> {
    ...
    return {
      ...
      notice:
        "Simulation only. Entries and exits use read-only Jupiter quotes and never build, sign, or submit a transaction.",
    };
  }
}
