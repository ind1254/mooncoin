...
export type RiskDirection = "increases" | "decreases" | "neutral";

export interface RiskFactor {
  id: string;
  label: string;
  /** The observation, stated neutrally. No interpretation here. */
  fact: string;
  /** What it may imply. Kept separate from the fact on purpose. */
  interpretation: string;
  /** Points contributed to the risk score. Always explicit, never hidden. */
  points: number;
  direction: RiskDirection;
  /** Provenance of the evidence behind this factor. */
  status: EvidenceStatus;
  source: string;
}

export interface RiskAssessment {
  /** 0-100. Higher is riskier. Never an opportunity or quality score. */
  riskScore: number;
  /**
   * 0-100. How much evidence actually backs the score. A 20 with confidence 90
   * and a 20 with confidence 25 are very different statements, and the UI must
  ...
  factors: RiskFactor[];
  /** Dotted evidence paths this assessment wanted and did not have. */
 ...
 */
const AUTHORITY_WEIGHT: Record<EvidenceStatus, number> = {
  verified: 1,
  reported: 0.8,
  derived: 0.8,
  stale: 0.5,
...
const CONFIDENCE_WEIGHT: Record<EvidenceStatus, number> = {
  verified: 1,
  reported: 0.8,
  derived: 0.75,
  stale: 0.35,
...
function evaluate<T>(
  ...
  label: string,
  evidence: Evidence<T>,
  present: (value: T) => FactorOutcome,
  absent: FactorOutcome,
): { factor: RiskFactor; confidence: number; maxConfidence: number } {
  const usable = hasValue(evidence);
  const outcome = usable ? present(evidence.value as T) : absent;
...
const pct = (bps: bigint): string => `${(Number(bps) / 100).toFixed(1)}%`;
const usd = (micro: bigint): string =>
  `$${(Number(micro) / 1_000_000).toLocaleString("en-US", { maximumFractionDigits: 0 })}`;

export interface RiskPolicy {
  /** Liquidity below this is treated as unable to absorb a normal exit. */
  minLiquidityUsdMicro: bigint;
  /** Price impact above this makes an exit materially expensive. */
...
export function assessRisk(
  snapshot: TokenEvidenceSnapshot,
  policy: RiskPolicy = DEFAULT_RISK_POLICY,
): RiskAssessment {
  const results: Array<ReturnType<typeof evaluate>> = [];
  ...
  results.push(
    evaluate(
      "mint_authority",
      "Mint authority",
      snapshot.authorities.mintAuthorityRevoked,
      (revoked) =>
        revoked
  ...
  results.push(
    evaluate(
      "freeze_authority",
      "Freeze authority",
      snapshot.authorities.freezeAuthorityRevoked,
      (revoked) =>
        revoked
  ...
  results.push(
    evaluate(
      "provider_agreement",
      "Provider agreement",
      snapshot.authorities.providerAgreement,
      (agreement) =>
        agreement === "disagrees"
          ...
          : agreement === "agrees"
            ...
            : { points: 2, fact: "The provider did not report authority status.", interpretation: "Nothing to cross-check against the chain.", direction: "neutral" },
      { points: 3, fact: "Provider agreement was not assessed.", interpretation: "No chain read was available to compare against.", direction: "neutral" },
    ),
  );
  ...
  results.push(
    evaluate(
      ...
      snapshot.holders.topWalletConcentrationBps,
      (bps) => {
        const points = bps >= 6_000n ? 24 : bps >= 4_000n ? 18 : bps >= 2_500n ? 11 : bps >= 1_500n ? 5 : 0;
        return {
          points,
  ...
  results.push(
    evaluate(
      ...
      snapshot.holders.unclassifiedBps,
      (bps) =>
        bps > 0n
          ...
          : { points: 0, fact: "All top holders were classified.", interpretation: "The concentration figure is complete.", direction: "decreases" },
      { points: 4, fact: "Holder classification was not attempted.", interpretation: "Pool-held supply cannot be separated from wallet-held supply." },
    ),
  );
  ...
  results.push(
    evaluate(
      "liquidity_depth",
      "Liquidity",
      snapshot.liquidity.liquidityUsdMicro,
      (micro) => {
  ...
  results.push(
    evaluate(
      "ticker_ambiguity",
      "Ticker ambiguity",
      snapshot.identity.duplicateSymbolCount,
      (count) =>
        count > 1
          ...
          : { points: 0, fact: "This ticker resolves to a single mint.", interpretation: "No identity ambiguity for this symbol.", direction: "decreases" },
      { points: 3, fact: "Ticker ambiguity was not measured.", interpretation: "Whether other mints share this symbol is unknown.", direction: "neutral" },
    ),
  );
  ...
  results.push(
    evaluate(
      "market_freshness",
      "Data freshness",
      snapshot.freshness.marketAgeMs,
      (ageMs) => {
