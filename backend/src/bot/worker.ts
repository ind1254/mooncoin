...
import type {
  LivePaperPositionRecord,
  LivePaperPositionRepository,
  PaperBotConfigRepository,
  PaperBotDecisionRepository,
  PaperBotPositionStateRepository,
} from "../db/repositories.js";
import type { LiveTokenFeedProvider, LiveFeedToken } from "../market/jupiter/liveFeed.js";
import type { QuoteProvider, NormalizedSwapQuote } from "../market/jupiter/quotes.js";
import { USDC_MINT } from "../market/tradability.js";
import type { LivePaperTradingService } from "../paper/livePaper.js";
import {
  assessPaperBotCandidate,
  duplicateSymbolCounts,
  evaluatePaperBotExit,
  type PaperBotCandidate,
} from "./strategy.js";
import type { PaperBotConfigRecord, PaperBotDecisionAction } from "./types.js";

type ConfigStore = Pick<PaperBotConfigRepository, "listEnabled" | "markRun">;
type PositionStore = Pick<LivePaperPositionRepository, "listOpenForBot" | "hasBotPositionSince">;
type StateStore = Pick<PaperBotPositionStateRepository, "get" | "recordValue" | "markExited">;
type DecisionStore = Pick<PaperBotDecisionRepository, "create" | "hasRecentAction">;
type BotPaperService = Pick<LivePaperTradingService, "openPosition" | "closePosition">;

export interface PaperBotWorkerDeps {
  configs: ConfigStore;
  positions: PositionStore;
  states: StateStore;
  decisions: DecisionStore;
  feed: Pick<LiveTokenFeedProvider, "getFeed">;
  quotes: Pick<QuoteProvider, "getQuote">;
  createPaperTrading: (config: PaperBotConfigRecord) => BotPaperService;
  maxMarketAgeMs: number;
  clock?: () => number;
  log?: (line: Record<string, unknown>) => void;
}

export interface PaperBotPassSummary {
...
function ensureExitQuote(position: LivePaperPositionRecord, quote: NormalizedSwapQuote, nowMs: number): void {
  if (
...
function candidateSnapshot(candidate: PaperBotCandidate): Record<string, unknown> {
...
async function recordDeduped(
  deps: PaperBotWorkerDeps,
  config: PaperBotConfigRecord,
  action: PaperBotDecisionAction,
  tokenMint: string | null,
  input: Parameters<DecisionStore["create"]>[0],
  nowMs: number,
): Promise<void> {
  if (await deps.decisions.hasRecentAction(config.id, tokenMint, action, nowMs - DECISION_DEDUP_MS)) return;
...
async function evaluateOpenPositions(
  deps: PaperBotWorkerDeps,
  config: PaperBotConfigRecord,
  summary: PaperBotPassSummary,
  nowMs: number,
): Promise<{ open: LivePaperPositionRecord[]; degraded: boolean }> {
  ...
  for (const position of open) {
    ...
    try {
      ...
      await deps.states.recordValue(
        position.id,
        config.id,
        exit.highWaterValueMicroUsd,
        quote.minOutAmount,
        nowMs,
      );
      if (!exit.shouldClose || !exit.reason) continue;
...
function candidatesFor(
  ...
  return tokens
    .map((token) =>
      assessPaperBotCandidate(
        token,
        duplicates.get(token.token.symbol.trim().toLowerCase()) ?? 1,
        config,
        maxMarketAgeMs,
        nowMs,
      ),
    ...
    .sort(
      (a, b) =>
        b.assessment.qualityScore - a.assessment.qualityScore ||
...
async function attemptEntry(
  deps: PaperBotWorkerDeps,
  config: PaperBotConfigRecord,
  currentOpen: LivePaperPositionRecord[],
  tokens: LiveFeedToken[],
  summary: PaperBotPassSummary,
  nowMs: number,
): Promise<{ opened: boolean; rejected: number }> {
  if (currentOpen.length >= config.maxOpenPositions) return { opened: false, rejected: 0 };
  ...
  for (const candidate of candidates) {
    if (attempted >= MAX_ENTRY_ATTEMPTS_PER_CONFIG) break;
    const mint = candidate.token.token.mint;
    if (openMints.has(mint)) continue;
    if (
      await deps.positions.hasBotPositionSince(
        config.id,
        mint,
        nowMs - config.cooldownMinutes * 60_000,
      )
    ) continue;

    attempted++;
    try {
      const position = await deps.createPaperTrading(config).openPosition(
        config.userId,
        mint,
        microToUsdString(config.tradeSizeMicroUsd),
        config.slippageBps,
        randomUUID(),
        {
          openedBy: "paper_bot",
      ...
      if (!positionId) throw new ArbError("INTERNAL_ERROR", "Paper entry did not return a position id.", 500);
      await deps.decisions.create({
        configId: config.id,
        positionId,
        tokenMint: mint,
...
export async function runPaperBotPass(deps: PaperBotWorkerDeps): Promise<PaperBotPassSummary> {
  const startedAt = deps.clock?.() ?? Date.now();
  const summary: PaperBotPassSummary = {
    configsProcessed: 0,
  ...
  const state = new Map<string, { open: LivePaperPositionRecord[]; degraded: boolean }>();
