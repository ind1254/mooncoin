...
import { runScheduledWorkerPass, type ScheduledWorkerPassResult } from "./pass.js";

export interface ScheduledWorkerRuntime {
  run(runKey: string): Promise<ScheduledWorkerPassResult>;
  close(): Promise<void>;
}

export interface CreateScheduledWorkerRuntimeOptions {
  db?: SqlClient;
  clock?: () => number;
...
export function createScheduledWorkerRuntime(
  env: AppEnv,
  options: CreateScheduledWorkerRuntimeOptions = {},
): ScheduledWorkerRuntime {
  ...
  const research = new ResearchService(
    ...
    { clock, mintCacheTtlMs: env.MINT_CACHE_TTL_MS },
  );
