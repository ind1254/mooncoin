...
/**
 * Bounded token history — the durable series behind "what was risk an hour
 * ago?" and "what changed while I held this?".
 *
 * The retention policy is the feature, not an afterthought. Appending forever
 * would be trivial and would quietly become the largest table in the database;
 * downsampling keeps a token to a few hundred rows at steady state while
 * preserving the shape of its past. See migration 013 for the tiers.
 *
 * Kept in its own module rather than added to repositories.ts: that file is
 * already long, and history has a distinct lifecycle (a prune pass) that the
...
export interface TokenHistoryPoint {
  ...
  pricePicoUsd: bigint | null;
  liquidityUsdMicro: bigint | null;
  marketCapUsdMicro: bigint | null;
  volume24hUsdMicro: bigint | null;
  walletConcentrationBps: bigint | null;
  programHeldBps: bigint | null;
...
export interface HistoryPruneResult {
  downsampledToMedium: number;
  downsampledToLow: number;
  deleted: number;
}
...
function mapRow(row: SqlRow): TokenHistoryPoint {
  return {
    tokenMint: str(row.token_mint),
    observedAtMs: dateMs(row.observed_at),
    resolution: str(row.resolution) as "high" | "medium" | "low",
...
export class TokenHistoryRepository {
   ...
   */
  async record(point: TokenHistoryPoint): Promise<void> {
    await this.db.query(
  ...
  async list(tokenMint: string, limit = 200): Promise<TokenHistoryPoint[]> {
    const rows = await this.db.query(
      `${SELECT_COLUMNS} where token_mint = $1 order by observed_at desc limit $2`,
      [tokenMint, limit],
    );
   ...
   */
  async asOf(tokenMint: string, atMs: number): Promise<TokenHistoryPoint | null> {
    const rows = await this.db.query(
      `${SELECT_COLUMNS}
        where token_mint = $1
  ...
  async between(tokenMint: string, fromMs: number, toMs: number): Promise<TokenHistoryPoint[]> {
    const rows = await this.db.query(
      `${SELECT_COLUMNS}
        where token_mint = $1
          and observed_at >= to_timestamp($2::double precision / 1000)
   ...
   */
  async prune(nowMs: number): Promise<HistoryPruneResult> {
    ...
    await this.db.query(
      `update token_history set resolution = 'medium'
    ...
    await this.db.query(
      `update token_history set resolution = 'low'
