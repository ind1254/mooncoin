...
import {
  PAPER_BOT_STRATEGY_VERSION,
  type CreatePaperBotDecisionInput,
  type PaperBotConfigRecord,
  type PaperBotDecisionAction,
  type PaperBotDecisionRecord,
  type PaperBotPositionStateRecord,
  type PaperBotStrategyConfig,
} from "../bot/types.js";
...
function toLivePaperPosition(row: SqlRow): LivePaperPositionRecord {
  ...
  if (openedBy !== "manual" && openedBy !== "paper_bot") throw new Error("Unexpected paper-position origin");
  return {
...
export class LivePaperPositionRepository {
  ...
  async listOpenForBot(configId: string): Promise<LivePaperPositionRecord[]> {
    const rows = await this.db.query(
      `select pp.*
         from paper_positions pp
  ...
  async hasBotPositionSince(configId: string, tokenMint: string, sinceMs: number): Promise<boolean> {
    const rows = await this.db.query(
      `select 1
  ...
  async open(
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (input.openedBy === "paper_bot") {
        ...
        const botConfig = await tx.query(
          `select id from paper_bot_configs
            where id = $1 and user_id = $2 and enabled = true
            for update`,
          [input.botConfigId, userId],
        );
        if (!botConfig[0]) {
          throw new ArbError("PAPER_BOT_DISABLED", "The paper bot was disabled before this entry could open.", 409);
        }
        const botCounts = await tx.query<{ count: string }>(
          `select count(*)::text as count
             from paper_positions
            where bot_config_id = $1 and opened_by = 'paper_bot' and status = 'open'`,
          [input.botConfigId],
        );
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (botConfigId !== null) {
        ...
        const enabled = await tx.query(
          "select id from paper_bot_configs where id = $1 and user_id = $2 and enabled = true for update",
          [botConfigId, userId],
        );
        if (!enabled[0]) {
...
function toPaperBotConfig(row: SqlRow): PaperBotConfigRecord {
  const strategyVersion = readString(row.strategy_version);
  if (strategyVersion !== PAPER_BOT_STRATEGY_VERSION) throw new Error("Unexpected paper-bot strategy version");
  ...
  return {
    ...
    lastRunStatus: lastRunStatus as PaperBotConfigRecord["lastRunStatus"],
...
export class PaperBotConfigRepository {
  ...
  async ensureDefault(userId: string, nowMs: number): Promise<PaperBotConfigRecord> {
    const rows = await this.db.query(
      `insert into paper_bot_configs (user_id, created_at, updated_at)
  ...
  async findById(configId: string): Promise<PaperBotConfigRecord | null> {
  ...
  async listEnabled(limit = 100): Promise<PaperBotConfigRecord[]> {
    const rows = await this.db.query(
      `select * from paper_bot_configs
        where enabled = true
  ...
  async save(
    ...
    nowMs: number,
  ): Promise<PaperBotConfigRecord> {
    const rows = await this.db.query(
      `update paper_bot_configs set
         enabled = $2,
    ...
    if (!rows[0]) throw new ArbError("PAPER_BOT_NOT_FOUND", "Paper-bot settings were not initialized.", 404);
    return toPaperBotConfig(rows[0]);
  }
  ...
  async markRun(
    ...
    await this.db.query(
      `update paper_bot_configs set
       ...
       where id = $1`,
      [configId, nowMs, status, summary],
    );
  }
}

function toPaperBotDecision(row: SqlRow): PaperBotDecisionRecord {
  const action = readString(row.action);
  const actions: PaperBotDecisionAction[] = [
    "opened",
    "entry_rejected",
    "closed",
    "exit_unavailable",
  ...
  if (!actions.includes(action as PaperBotDecisionAction)) throw new Error("Unexpected paper-bot decision action");
  return {
    id: readString(row.id),
    ...
    action: action as PaperBotDecisionAction,
...
export class PaperBotDecisionRepository {
  ...
  async create(input: CreatePaperBotDecisionInput): Promise<PaperBotDecisionRecord> {
    const rows = await this.db.query(
      `insert into paper_bot_decisions (
  ...
  async listForUser(userId: string, limit = 30): Promise<PaperBotDecisionRecord[]> {
    const rows = await this.db.query(
      `select d.*
  ...
  async hasRecentAction(
    ...
    const rows = await this.db.query(
      `select 1 from paper_bot_decisions
        where config_id = $1
          and token_mint is not distinct from $2
          and action = $3
          and created_at >= to_timestamp($4::double precision / 1000)
        limit 1`,
      [configId, tokenMint, action, sinceMs],
    );
    return rows.length > 0;
  }
}

function toPaperBotPositionState(row: SqlRow): PaperBotPositionStateRecord {
  return {
...
export class PaperBotPositionStateRepository {
  ...
  async get(positionId: string): Promise<PaperBotPositionStateRecord | null> {
  ...
  async recordValue(
    positionId: string,
    configId: string,
    highWaterMicroUsd: bigint,
    valueMicroUsd: bigint,
    nowMs: number,
  ): Promise<PaperBotPositionStateRecord> {
    const rows = await this.db.query(
      `insert into paper_bot_position_state (
       ...
       on conflict (position_id) do update set
         ...
         updated_at = excluded.updated_at
       returning *`,
      [positionId, configId, highWaterMicroUsd.toString(), valueMicroUsd.toString(), nowMs],
    );
    return toPaperBotPositionState(rows[0]!);
  }

  async markExited(positionId: string, reason: string, nowMs: number): Promise<void> {
    await this.db.query(
      `update paper_bot_position_state
          set exit_reason = left($2, 120), updated_at = to_timestamp($3::double precision / 1000)
        where position_id = $1`,
      [positionId, reason, nowMs],
    );
  }
}
