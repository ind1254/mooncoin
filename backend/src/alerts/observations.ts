...
import type { AlertObservation } from "./engine.js";

/**
 * Turning consecutive snapshots of a token into the deltas alerts compare.
 *
 * Pure, like the engine: snapshots in, observation out.
 */
 ...
 */
export function changeBps(from: bigint | null, to: bigint | null): bigint | null {
  ...
  const bps = (magnitude * 10_000n + from - 1n) / from;
  return negative ? -bps : bps;
}

/** Reads the facts the worker persists out of a full research profile. */
export function snapshotFromProfile(profile: ResearchProfile, nowMs: number): TokenSnapshot {
  ...
  return {
    ...
    volume24hUsdMicro:
      profile.market.buyVolume24hUsdMicro === null || profile.market.sellVolume24hUsdMicro === null
        ? null
    ...
    walletConcentrationBps:
      holders && holders.status !== "unavailable" && holders.concentrationBps !== undefined
...
export function buildObservation(
  ...
  const comparable =
    previous !== null && intervalMs !== null && intervalMs > 0 && intervalMs <= MAX_DIFF_AGE_MS;
