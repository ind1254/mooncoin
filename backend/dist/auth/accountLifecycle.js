...
import { AccountActionTokenRepository, UserRepository, } from "../db/repositories.js";
...
export class AccountLifecycleService {
    ...
    async sendVerification(userId) {
        ...
        if (!user)
            throw new ArbError("UNAUTHORIZED", "Account not found", 401);
        if (user.emailVerifiedAtMs !== null)
            return { sent: false, reason: "already_verified" };
        if (!this.sender)
            return { sent: false, reason: "delivery_unconfigured" };
        const rawToken = actionToken();
        const now = this.clock();
        const actionId = await this.actions.issue(user.id, "verify_email", rawToken, now, now + this.verificationTtlMs);
        ...
        await this.sender.send({
            ...
            html: emailHtml("Verify your email", `Confirm ${escapeHtml(user.email)} to protect your Moonpaper paper-trading account.`, "Verify email", link, "This link expires in 24 hours. Moonpaper never asks for a wallet or private key."),
    ...
    async requestPasswordReset(email) {
        ...
        const rawToken = actionToken();
        const now = this.clock();
        const actionId = await this.actions.issue(user.id, "reset_password", rawToken, now, now + this.resetTtlMs);
        ...
        await this.sender.send({
            ...
            html: emailHtml("Reset your password", "A password reset was requested for your Moonpaper account.", "Reset password", link, "This link expires in 60 minutes. If you did not request it, you can ignore this email."),
...
function emailHtml(title, intro, button, link, footer) {
