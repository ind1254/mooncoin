...
export class PaperTradingEngine {
    ...
    clock;
    state;
    constructor(market, store, clock = Date.now, config = DEFAULT_ENGINE_CONFIG) {
        this.market = market;
        this.store = store;
        this.clock = clock;
    ...
    async openPosition(req) {
        ...
        if (req.solAmountLamports > this.state.cashLamports) {
            throw new ArbError("INSUFFICIENT_PAPER_BALANCE", "Not enough virtual SOL for this paper trade", 409, { availableLamports: this.state.cashLamports.toString() });
        }
        ...
        if (best.priceImpactBps > req.limits.maxPriceImpactBps) {
            throw new ArbError("PRICE_IMPACT_TOO_HIGH", `Estimated price impact ${(Number(best.priceImpactBps) / 100).toFixed(2)}% exceeds your ${(Number(req.limits.maxPriceImpactBps) / 100).toFixed(2)}% limit`, 409);
        }
        ...
        if (totalCost > this.state.cashLamports) {
            throw new ArbError("INSUFFICIENT_PAPER_BALANCE", "Not enough virtual SOL for this paper trade including fees", 409, { requiredLamports: totalCost.toString(), availableLamports: this.state.cashLamports.toString() });
        }
        ...
        const scores = computeScores(view, routes, (req.solAmountLamports * view.solPriceMicroUsd) / LAMPORTS_PER_SOL, req.limits, nowMs);
        const position = {
            id: randomUUID(),
    ...
    applyValuation(p, sell, nowMs) {
        ...
        if (sell.outAmount > p.highWaterLamports)
            p.highWaterLamports = sell.outAmount;
        if (sell.outAmount < p.lowWaterLamports)
            p.lowWaterLamports = sell.outAmount;
...
export function computeStats(open, closed) {
    ...
    const networkFees = [...open, ...closed].reduce((s, p) => s + p.entryNetworkFeeLamports + (p.exitNetworkFeeLamports ?? 0n), 0n);
