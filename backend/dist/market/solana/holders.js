 ...
 */
function shareBps(amountBaseUnits, supplyBaseUnits) {
    if (supplyBaseUnits <= 0n || amountBaseUnits <= 0n)
        return 0n;
 ...
 */
export async function readHolderConcentration(client, mint, supplyBaseUnits, signal) {
    const empty = {
        concentrationBps: 0n,
        programHeldBps: 0n,
        walletHolderCount: 0,
    ...
    if (funded.length === 0 || supplyBaseUnits <= 0n)
        return empty;
    ...
    const accounts = await client.getMultipleAccounts(funded.map((entry) => entry.address), signal);
    ...
    const holders = owners.map((owner, index) => {
        ...
        if (ownerAccount === null) {
            return { owner, amountBaseUnits, kind: "unknown" };
        }
        const kind = ownerAccount.owner === SYSTEM_PROGRAM_ID ? "wallet" : "program";
    ...
    const sum = (list) => list.reduce((total, holder) => total + holder.amountBaseUnits, 0n);
    const unknownAmount = unclassifiedAmount + sum(holders.filter((holder) => holder.kind === "unknown"));
    return {
