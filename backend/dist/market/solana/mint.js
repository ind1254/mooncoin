...
function readCOptionTag(view, offset) {
 ...
 */
export function parseMintAccount(data, owner) {
    if (owner === TOKEN_2022_PROGRAM_ID) {
        return {
            status: "unsupported_program",
            owner,
            reason: "Token-2022 mint. Its first 82 bytes match the classic layout but extensions follow; " +
    ...
    if (data.byteLength !== MINT_ACCOUNT_LEN) {
        const hint = data.byteLength === TOKEN_ACCOUNT_LEN
...
export async function readMintAccount(client, mintAddress, signal) {
