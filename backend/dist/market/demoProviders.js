import { DEMO_CREATED_AT, DEMO_SOL_PRICE_MICRO_USD, DEMO_TOKENS, DEMO_VENUES, } from "./demoData.js";
import { bucketOf, sampleRange } from "./prng.js";
import { applyHaircutFloor, divCeil, LAMPORTS_PER_SOL } from "../core/money.js";
/**
...
function scenarioOf(mint) {
    return DEMO_TOKENS.find((t) => t.mint === mint);
}
/** Simulated mid price at a moment, pico-USD. Continuous & deterministic. */
export function demoPricePicoUsd(s, atMs) {
    const phase = (k) => sampleRange(`${s.mint}:phase:${k}`, 0, Math.PI * 2);
    const wave = (periodMs, ampBps, k) => Math.sin((atMs / periodMs) * Math.PI * 2 + phase(k)) * ampBps;
    ...
    const totalBps = wave(6 * HOUR_MS, s.waveSlowBps, "slow") +
        wave(45 * 60_000, s.waveMediumBps, "med") +
        wave(4 * 60_000, s.waveFastBps, "fast") +
        trend;
    const factor = 1 + totalBps / 10_000;
    const pico = (s.basePricePicoUsd * BigInt(Math.round(factor * 1_000_000))) / 1000000n;
    return pico > 0n ? pico : 1n;
}
function changeBps(nowPico, thenPico) {
    if (thenPico <= 0n)
        return 0n;
    return ((nowPico - thenPico) * 10000n) / thenPico;
}
function reliabilityFor(ageMs, freshLimitMs) {
    return ageMs <= freshLimitMs ? "fresh" : "stale";
}
function point(value, observedAtMs, nowMs, freshLimitMs) {
...
function jitter(value, maxBps, key) {
...
export class DemoMarket {
    ...
    async getSolPriceMicroUsd() {
        ...
        return (DEMO_SOL_PRICE_MICRO_USD * BigInt(Math.round((1 + wave / 10_000) * 1_000_000))) / 1000000n;
    }
    ...
    async routes(mint, side, amountIn, slippageBps) {
        const s = this.requireScenario(mint);
        if (amountIn <= 0n) {
        ...
        for (const venue of DEMO_VENUES) {
            ...
            const venuePricePico = midPico + (midPico * offsetBps) / 10000n;
            const venueLiqMicro = (liquidity * venue.liquidityShareBps) / 10000n;
            ...
            if (side === "buy") {
                ...
                const impactBps = clampBps(divCeil(usdMicroIn * 2n * 10000n, venueLiqMicro));
                const grossTokens = (usdMicroIn * 1000000n * tokenScale) / venuePricePico;
                ...
                quotes.push({
                    ...
                    effectivePricePicoUsd: (usdMicroIn * 1000000n * tokenScale) / out,
                ...
                const usdMicroGross = (amountIn * venuePricePico) / tokenScale / 1000000n;
                const impactBps = clampBps(divCeil(usdMicroGross * 2n * 10000n, venueLiqMicro));
                ...
                quotes.push({
                    ...
                    outAmount: out,
                    minReceived: applyHaircutFloor(out, slippageBps),
                    effectivePricePicoUsd: (usdMicroGross * 1000000n * tokenScale) / amountIn,
                    priceImpactBps: impactBps,
                    routeFeeBps: venue.routeFeeBps,
                    networkFeeLamports: networkFee,
                    priorityFeeLamports: priorityFee,
...
export function createDemoBundle(clock = Date.now) {
