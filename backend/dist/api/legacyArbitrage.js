...
import { AMOUNT_LIMITS, getEnabledToken, getEnabledVenue, listTokens, listVenues, setTokenEnabled, setVenueEnabled, } from "../config/allowlist.js";
 ...
 */
export function createLegacyArbitrageRouter(useMockQuotes, adminToken) {
    const router = express.Router();
    const store = new InMemoryPaperStore();
    function buildAdapters() {
        if (useMockQuotes) {
            return [new MockVenueAdapter("raydium", 1400n), new MockVenueAdapter("orca", 1420n)];
        }
        return Object.entries(VENUE_DEX_LABELS).map(([venueId, labels]) => new JupiterVenueAdapter(venueId, labels));
    }
    ...
    router.post("/v1/arbitrage/calculate", async (req, res) => {
        ...
        try {
            ...
            if (startingAmountUsd < AMOUNT_LIMITS.minUsd || startingAmountUsd > AMOUNT_LIMITS.maxUsd) {
                throw new ArbError("AMOUNT_OUT_OF_RANGE", `Amount must be between $${AMOUNT_LIMITS.minUsd} and $${AMOUNT_LIMITS.maxUsd}`, 400);
            }
