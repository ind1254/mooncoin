...
export function isAuthorizedOwnerRequest(authorization, apiKey) {
    if (!authorization || !apiKey || apiKey.length < 32)
        return false;
...
export async function resolveUser(req, auth, db, ownerAccess) {
    ...
    try {
        if (token) {
            ...
            if (sessionUser && isDesignatedOwner)
                return sessionUser;
        }
...
export function createAuthRouter(options) {
    ...
    const { getAuth, getDb, getAccountLifecycle, createPaperTrading, startingMicroUsd, secureCookies, emailVerificationRequired, ownerAccess, clock, rateLimits, } = options;
    ...
    const encodeSequenceCursor = (decision) => Buffer.from(`${decision.createdAtMs}:${decision.id}`, "utf8").toString("base64url");
    ...
    const serializeFomoSequence = (decision, config) => ({
    ...
    const passwordEntryAllowed = (_req, res, next) => {
        if (ownerAccess) {
            res.status(403).json({
                error: "FORBIDDEN",
    ...
    router.post("/v1/owner/unlock", requirePersistence, async (req, res) => {
        try {
            await consumeLimit(res, "owner:network", req.ip ?? req.socket.remoteAddress ?? "unresolved-network", rateLimits.authNetworkAttempts, rateLimits.authWindowMs);
    ...
    router.get("/v1/integrations/fomo/sequences", requireAuth, async (req, res) => {
        try {
            ...
            const user = currentUser(res);
            await consumeLimit(res, "integration:sequence-reads", user.id, rateLimits.integrationAttempts, rateLimits.integrationWindowMs);
            const configs = new PaperBotConfigRepository(getDb());
