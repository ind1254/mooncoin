...
function scenario(symbol) {
    const s = DEMO_TOKENS.find((t) => t.symbol === symbol);
    if (!s)
        throw new Error(`demo scenario missing: ${symbol}`);
    return s;
}
/** Simulated tokens bought with `sol` at time `atMs`, net of ~0.6% costs. */
function tokensFor(symbol, solLamports, atMs, solPriceMicro) {
    const s = scenario(symbol);
    const price = demoPricePicoUsd(s, atMs);
    const usdMicro = (solLamports * solPriceMicro) / LAMPORTS_PER_SOL;
    const gross = (usdMicro * 1000000n * 10n ** BigInt(s.decimals)) / price;
    return (gross * 9940n) / 10000n;
}
export function seedDemoState(engine, notify, nowMs) {
    const state = engine.getState();
    if (state.positions.length > 0)
        return; // only seed a fresh install
    const solPriceMicro = 150000000n;
    ...
    const bonkFees = 590000n;
    const bonkCost = sol(12) + bonkFees;
    const bonkExit = (bonkCost * 10840n) / 10000n;
    const closedWin = {
        ...base,
        id: randomUUID(),
        tokenMint: bonk.mint,
        ...
        returnBps: 840n,
        highWaterLamports: (bonkCost * 10910n) / 10000n,
        lowWaterLamports: (bonkCost * 9960n) / 10000n,
        lastValuedAtMs: bonkClosedAt,
        closedAtMs: bonkClosedAt,
        exitVenueId: "orca",
        exitVenueName: "Orca",
        exitValueLamports: bonkExit,
        exitNetworkFeeLamports: 480000n,
        exitImpactBps: 4n,
    ...
    const floofFees = 710000n;
    const floofCost = sol(3) + floofFees;
    const floofExit = (floofCost * 7650n) / 10000n;
    const closedLoss = {
        ...base,
        id: randomUUID(),
        ...
        unrealizedPnlLamports: 0n,
        returnBps: -2350n,
        highWaterLamports: (floofCost * 10600n) / 10000n,
        lowWaterLamports: (floofCost * 7400n) / 10000n,
        lastValuedAtMs: floofClosedAt,
        closedAtMs: floofClosedAt,
        exitVenueId: "raydium",
        exitVenueName: "Raydium",
        exitValueLamports: floofExit,
        exitNetworkFeeLamports: 830000n,
        exitImpactBps: 940n,
    ...
    notify.push({
        ...
        reason: "Opportunity quality is high with low risk: trading volume is up sharply versus the previous hour, liquidity is deep and stable, and estimated price impact for a 10 SOL paper trade is below your 1% limit. This describes current conditions, not a prediction.",
