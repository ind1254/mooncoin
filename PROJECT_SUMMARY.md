...
- **Deployed:** https://mooncoin-two.vercel.app/
...
Also: per-account settings with sensible defaults, and database-backed in-app
notifications with cooldowns and material-change gates where every alert
explains why it fired ("BONK now matches your balanced strategy: liquidity
increased, ..."). These restore with the user's watchlist, Bot Lab decisions,
and paper portfolio after every sign-in.
...
- **Testing:** 467 passing test executions across 43 suite files: live-feed normalization, concentrated live-v2
...
- **Account system is portfolio-grade, not identity-platform complete.** Password
  sessions, recovery, optional email verification, persistent account state,
  and scheduled workers are implemented; social OAuth, MFA, and device/session
  management UI are not.
- **The iOS module belongs to an earlier product phase.** CI contains the macOS
  build workflow, but the web app remains the reference paper-trading UI.
- **Origin:** started as an arbitrage-calculator add-on concept for a
  third-party trading app ("Fomo"), then pivoted to a standalone product. The
  legacy functionality remains only in the explicitly labeled Demo Sandbox.
...
- **466 declared test cases** across 43 suites, all passing; strict-mode TypeScript
- **50 HTTP routes** (market, auth, live/demo paper trading, settings, bot, legacy/admin)
- **15 forward-only Postgres migrations**, shared by production and local tests
- **72 backend TypeScript source files**
<!-- METRICS:END -->
...
- What would production need next? (push notifications, social OAuth/MFA, a
  durable chain indexer, a real risk-data
